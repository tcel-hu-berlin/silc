...
clickRect.on("touchstart", function(){
    startTouchX = d3.mouse(this)[0]
    startTouchY = d3.mouse(this)[1]
    startedMoving=false
    MoveCursor(startTouchX, startTouchY)
})
clickRect.on("touchmove", function(){
    let x = d3.mouse(this)[0], y = d3.mouse(this)[1]
    let xbounds = xP.range(), ybounds = yP.range()

    if ((x-xbounds[0])*(x-xbounds[1])<0 && (y-ybounds[0])*(y-ybounds[1])<0){
        DragCursorTouch(x, y)
    } else {
...
clickRect.on("mousemove", function(){
    var x = d3.mouse(this)[0]
    var y = d3.mouse(this)[1]
    MoveCursor(x, y)
})
...
svgP.append("text")
    ...
    .attr("x", -marginP.top - heightP/2 + 80)
    .text("Intensität (Anzahl Elektronen)")
